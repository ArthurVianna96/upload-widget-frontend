...
import { uploadToStorage } from "../http/upload-file-to-storage";
	...
	immer((set, get) => {
		...
		async function processUpload(uploadId: string) {
			...
			try {
				...
				updateUpload(uploadId, { status: "success", remoteUrl });
			} catch (error) {
...
export const usePendingUploads = () => {
	return useUploads(
		useShallow((store) => {
			...
			const progress = Math.min((Math.round(uploaded) * 100) / total, 100);

			return { isThereAnyPendingUploads, globalPercentage: progress };
		}),
	);
};
